// mcpdemo: a small gooey app that is also an MCP server.
//
...
func main() {
	...
	dir := "cmd/mcpdemo"
	if _, err := os.Stat(filepath.Join(dir, "mcpdemo.gooey")); err != nil {
		exe, _ := os.Executable()
		dir = filepath.Dir(exe)
	}

	app = gooey.NewApp(markup.Page(os.DirFS(dir), "mcpdemo.gooey", ctx))
