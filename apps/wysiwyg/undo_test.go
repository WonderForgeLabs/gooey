...
// TestCtrlShiftZIsUnfireableSoItIsNotBound is the test undo.go's header
// has named since it was written and this package has never had.
//
// The name is kept because the header's is, but writing it corrected
// the header: ctrl+shift+z is not unfireable, it is an ALIAS. Shift is
// not reportable on a printable character, so ParseGesture folds it
// into the rune and ctrl lower-cases it back — "ctrl+shift+z" and
// "ctrl+z" parse to the identical KeyEvent, which is also what the byte
// 0x1a decodes to.
//
// That makes a redo binding on it worse than dead, not better. Two
// KeyBindings on one gesture is the silent case: one never fires, and
// which one depends on tree order. A ctrl+shift+z "redo" would be a
// live candidate to shadow UNDO.
//
// THE ALIASING IS THE ASSERTION, and the absence is the consequence.
// Asserting only that the markup lacks the string would pass for a page
// that binds nothing at all, and would keep passing if ParseGesture
// changed underneath it.
func TestCtrlShiftZIsUnfireableSoItIsNotBound(t *testing.T) {
	plain, err := input.ParseGesture("ctrl+z")
	if err != nil {
		t.Fatal(err)
	}
	shifted, err := input.ParseGesture("ctrl+shift+z")
	if err != nil {
		t.Fatalf("ctrl+shift+z does not parse (%v), which would make it "+
			"a load error rather than a silent collision", err)
	}
	if shifted != plain {
		t.Fatalf("ctrl+shift+z parses to %v and ctrl+z to %v — they are "+
			"distinct gestures now, so a redo binding on the shifted one "+
			"is a real choice again and this test's reasoning is stale",
			shifted, plain)
	}

	// And that is the event the terminal actually delivers for both.
	ev, n, ok := input.Decode([]byte{0x1a}, false)
	if !ok || n != 1 {
		t.Fatalf("0x1a did not decode as one key event: ok=%v n=%d", ok, n)
	}
	if ev.Key != plain {
		t.Fatalf("0x1a decodes to %v, not %v — the collision above is "+
			"with something the terminal never sends", ev.Key, plain)
	}

	// AND, given that, the shipped page must not carry one.
	//
	// The attribute, NOT the bare string: the markup explains at length
	// why it does not bind ctrl+shift+z, so a substring check matches
	// the explanation and fails on a correct file. That is not
	// hypothetical — it is what this assertion did when it was first
	// written.
	src, err := os.ReadFile("wysiwyg.gooey")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(src), `Gesture="ctrl+z"`) {
		t.Fatal("the designer's markup binds no ctrl+z at all, so this " +
			"test is reading the wrong file or the wrong attribute")
	}
	if strings.Contains(string(src), `Gesture="ctrl+shift+z"`) {
		t.Error("the designer's markup binds ctrl+shift+z, which parses to " +
			"the same event as the ctrl+z Undo binding: one of the two " +
