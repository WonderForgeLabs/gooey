...
**Declare the `MenuBar` as the LAST child of its container.** What paints the dropdown above the content it drops over is that the surface is a `gooey.Overlay` — a layer above the whole page — so being last is not the mechanism; it ranks the dropdown against the page's *other* overlays, which is how a `ToastHost` or `AdornmentLayer` declared after it paints above an open menu ([#439](https://github.com/WonderForgeLabs/gooey/issues/439), [spec](specs/2026-09-01-overlay-hosts.md)). In a `Grid`, `Grid.Row="0"` still places the bar on the top row — element order and layout position are independent, which is exactly what an overlay needs.
...
Place it as the **last child of the root**, spanning the page (in a `Grid`, `Grid.RowSpan` across every row). What puts the toasts on top is that the host is a `gooey.Overlay` — a second paint layer above the whole page, which document order alone stopped buying when popup surfaces got one ([#439](https://github.com/WonderForgeLabs/gooey/issues/439), [spec](specs/2026-09-01-overlay-hosts.md)); declaring it last is what orders it against the *other* overlays. The host itself paints nothing and costs nothing while no toast is up; each toast is an ordinary leaf realized through the same structural re-sync a list uses, so showing one paints one component and dismissing one repaints exactly what it was covering.
...
Declare it as the **last child of the root**, spanning the page — the same hosting rule as `ToastHost`, and for the same reason: the layer is a `gooey.Overlay`, so it paints above the page whatever else is open, and its position among the *other* overlays is where it was declared. Declare it after the `ToastHost` and tooltips paint above toasts too ([#439](https://github.com/WonderForgeLabs/gooey/issues/439), [spec](specs/2026-09-01-overlay-hosts.md)). It takes no children and no attributes beyond `Name`: adornments attach themselves at runtime (a `Tooltip` finds the layer on its own), and code adds custom adorners through `Add`/`Remove`. The layer paints nothing, is transparent to the pointer, and re-anchors every adornment each frame, so a moved or resized target drags its adornments along and a target that leaves the tree or turns non-visible takes them down.
