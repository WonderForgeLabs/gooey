...
For an expression prefix, a declaration may sit on **any** element, not only on `<Gooey>`. Every such `xmlns:` attribute in the file is merged into one flat, document-wide table in document order, and the last declaration of a prefix wins. This is deliberately **not** XML's subtree scoping: a prefix declared on a button three levels down is usable by a sibling above it, and redeclaring a prefix lower in the file changes it for the whole document rather than for that subtree. Putting every declaration on `<Gooey>` is the convention and what every example here does; the rule matters because the designer does not always do it. The `<Gooey>` envelope is not part of the document model the designer edits, so an expression prefix it reads off the envelope is written back down onto the user's **root element**, and the files it saves depend on that being document-wide. What moves down is exactly that: a **prefixed expression** declaration. Everything else stays where the author wrote it — `xmlns:x`, which is an element prefix and needs XML scoping (below); the unprefixed default `xmlns`, which `carryDeclarations` never matches and the envelope keeps (`TestTheUnprefixedDeclarationStaysOnTheEnvelope`); and a prefix the file declares at both levels, where moving one would silently change which declaration wins.

That rule is about prefixes used inside **expressions** — handler namespaces and value namespaces, the ones that appear in an attribute's `{{…}}`. It does **not** cover `xmlns:x`: `x:` prefixes an *element* (`<x:Property>`), and element prefixes are resolved by Go's XML decoder with real subtree scoping before gooey sees them. So the declaration has to be **in scope at the `<x:Property>`**, which XML scoping makes two places: on `<Gooey>`, where one declaration serves every `<x:Property>` below it, or on the `<x:Property>` element itself. A declaration on a *sibling* — a `<Text>` elsewhere in the document — is never in scope for it, and the document is refused. That is the case the flat, document-wide rule would otherwise have let you expect to work.
...
**Unlike `<Menu>` and `<MenuItem>` below, an UNRECOGNIZED attribute on a `<Tab>` is still accepted.** `<Tab Frobnicate="yes">` loads — but `<Tab.Frobnicate>` does **not**: the property-element rule refuses every `<X.Foo>` on a pseudo-element, universal or not, because the spelling names a property of an element that has no builder to carry one. So the two spellings diverge exactly here, on the names the attribute rule tolerates. (The loader withholds the "put it on the content" advice for those, because it cannot know the destination takes them — it prescribes the attribute spelling on the same element instead, which asserts no destination at all. `<Tab.Header>` is why that matters: `Header` is the one attribute a `<Tab>` genuinely takes, and it is required.)

**An ATTACHED property is refused on any pseudo-element, whatever its surface.** `<Tab Grid.Row="1">` loaded, was dropped and reported nothing until [#486](https://github.com/WonderForgeLabs/gooey/pull/486); `Grid.Row` is an instruction to the element's *container* about a component, and a pseudo-element has none to instruct. That holds without knowing the element's own vocabulary, which is exactly why it can be said of a `<Tab>` while an unrecognized plain name cannot. The two rules look alike and are not: the menu elements declare an exhaustive attribute surface, so anything outside it is a typo the loader can name, while a `<Tab>`'s own vocabulary is whatever `<Tabs>` chooses to read and the catalog cannot enumerate it. What is refused here is the universal set **and every dotted name** — `Grid.Row`, `Grid.Col`, `Grid.RowSpan`, `Grid.ColSpan`, `Canvas.Left`, `Canvas.Top`, and any attached property a host registers — because both are answered by the element's structure rather than by its surface, and a pseudo-element has no structure to answer with. (That sentence said "the *universal* set specifically" until [#486](https://github.com/WonderForgeLabs/gooey/pull/486)'s review: that was the rule from before this widening, left inside the paragraph announcing it, and a reader who took it at face value concluded `<Tab Grid.Row="1">` still loads.)
...
| `MenuItem Separator` | A bool — `"true"`, `"1"`, `"T"` and the rest of Go's spellings — and anything unreadable is a load error rather than a silently ordinary item. It draws a rule instead of an item, and it carries **nothing else**: `Text`, `Gesture`, `Checked`, `Command`, `Icon` and `IconRune` on a separator are load errors. A separator draws none of them, and an accepted-then-ignored attribute is the silent drop this vocabulary exists to refuse. |
| `MenuItem Icon` / `IconRune` | The item's leading picture, in the framework's usual two tiers — **except that here the two tiers draw different things, not the same thing at two fidelities.** Elsewhere the cell-plane fallback is a halfblock rendering of the same image; a dropdown row is one cell tall, so that is two vertical samples for the whole glyph, and [#400](https://github.com/WonderForgeLabs/gooey/issues/400) measured two clearly different icons arriving as the same uniform `▀`. So `Icon` is drawn only where the terminal has a graphics protocol, and `IconRune` is what everywhere else shows — a glyph you choose, not a degraded picture. **Set both — an `Icon` without an `IconRune` is a load error.** The gutter is reserved whether or not a protocol exists, so an item carrying only the picture draws three blank columns on every terminal that has none, for the life of the program. `IconRune` alone is fine: the cell tier is a complete item, and only the pixel tier needs a partner. `Icon` is a **path in the page's own FS**, the same assets `<Image Src>` loads from, and unlike `Image` it takes no binding: `MenuItem.Icon` is a plain field read while painting, so a bound handle would be sampled once at load and never update, and it is refused at load rather than frozen at runtime. `IconRune` is exactly one glyph — a two-cell emoji is fine, two glyphs are a load error. A menu holding **any** icon gives *every* item a three-column gutter, whether or not a protocol exists, so the dropdown measures the same either way and does not reflow when the capability probe answers. |
...
**Declare the `MenuBar` wherever it belongs.** The dropdown is a `gooey.Overlay` — lifted out of document order into a paint layer of its own — so it paints above the content it drops over from anywhere in the tree, and within that layer it sits at `gooey.OverlayRankPopup`, the floor, so a toast or a tooltip raised over an open menu is still readable. In a `Grid`, `Grid.Row="0"` places the bar on the top row as usual.

*"Declare the `MenuBar` as the LAST child of its container"* is what this said until [#430](https://github.com/WonderForgeLabs/gooey/issues/430), which is the bug that shows why it was never the rule: being last bought being above the bar's own siblings, so a `MenuBar` on the wysiwyg designer canvas — beside a `Gauge`, an `ItemsView` and a `Border` — had its dropdown painted over on the very next repaint. Position is free now, for the click as well as for the paint: hit-testing asks the same overlay layer and the same ranks since [#465](https://github.com/WonderForgeLabs/gooey/issues/465). (An open menu holds the pointer capture anyway, so the bar never depended on either answer.)
...
Place it **anywhere in the tree**, spanning the page (in a `Grid`, `Grid.RowSpan` across every row). Position no longer decides the stacking: the host is a ranked overlay at `gooey.OverlayRankToast`, which is above the page and above any open popup, so a toast raised while a menu is open is still readable. "Declare it last" is what this used to say, and it was both unnecessary and — once popups moved to their own layer — insufficient; see [#439](https://github.com/WonderForgeLabs/gooey/issues/439).

**The freedom covers the click too.** It did not until [#465](https://github.com/WonderForgeLabs/gooey/issues/465): hit-testing was a separate walk that preferred later siblings and knew nothing about ranks, so a host declared **first** painted its toasts above a button and left the press to the button. Both walks now derive their answer from one function, so a toast that paints over something interactive also takes the press, wherever the host is declared. **Read that the other way too:** `Toast` is an ordinary leaf and is not hit-test-transparent — only the host is — so a toast you never meant anyone to click still swallows presses on whatever it covers, for as long as it is up. **And hovers**, which is the quieter half: `DispatchMouse` walks the tree once and derives both the press target and the hover target from that one answer, so the covered button also loses its highlight and its tooltip for the toast's lifetime — and unlike a swallowed click there is no gesture the user repeats and notices. A three-second toast over a button is three seconds of that button being unclickable and unhoverable. That is consistent with paint (whoever owns the cells owns the clicks in them) and it was already true before #465 whenever the host happened to be declared last; making `Toast` itself transparent is the alternative, and it is a behaviour change with its own semantics for toasts that *are* meant to be clicked — tracked as [#481](https://github.com/WonderForgeLabs/gooey/issues/481), which carries the three options and what each costs. The host itself paints nothing and costs nothing while no toast is up; each toast is an ordinary leaf realized through the same structural re-sync a list uses, so showing one paints one component and dismissing one repaints exactly what it was covering.
...
Place it **anywhere in the tree**, spanning the page — with one ordering caveat, below, which is narrower than it first reads and which no adornment this repo ships can reach. Tooltips paint above toasts, and it takes no ordering discipline to get that: the layer is a ranked overlay at `gooey.OverlayRankAdornment`, the top of the stack, because an adornment describes something already on screen and is useless underneath it. It takes no children and no attributes beyond `Name`: adornments attach themselves at runtime (a `Tooltip` finds the layer on its own), and code adds custom adorners through `Add`/`Remove`. The layer paints nothing, is transparent to the pointer, and re-anchors every adornment each frame, so a moved or resized target drags its adornments along and a target that leaves the tree or turns non-visible takes them down.

> **The caveat: a custom adornment that is neither a `components.PersistentAdornment` nor a `gooey.PointerFollower`, added before its anchor's first arrange, is orphaned and dropped, permanently** — not merely misplaced for a frame. Declaring the layer after the content it adorns is the one-word fix, and the only one: nothing in the framework catches the case for you.

This is where the hosting rule differs from `ToastHost`'s, and the difference is not about paint: layout walks children in document order (the overlay lift moves *paint* only), and the layer re-anchors during its own `Arrange` by reading each anchor's **current** bounds. An anchor that has not been arranged yet this frame has no bounds, which the layer cannot tell apart from an anchor that is gone. The two exemptions are exemptions because `Arrange` answers them before it asks about bounds at all — a persisting adornment whose anchor is in the tree is parked at its origin for that frame and placed on the next, and a pointer follower has no anchor to be early for. That is why the three shipped customers are all safe by construction rather than by discipline: a `Tooltip` adds its adornment on hover, long after the anchor has bounds; a `ValidationMarker`'s popup persists; a `DragGhost` follows the pointer. `components.TestAnAdornmentLayerDeclaredBeforeItsAnchorLosesTheAdornment` pins the drop and the ordering that causes it, and `components.TestAPersistentAdornmentSurvivesALayerDeclaredFirst` and `components.TestAPointerFollowerIsUnaffectedByDeclarationOrder` pin the two exemptions — without those two arms nothing goes red if the persist branch stops being taken, and the first would read as a rule about adornments in general. The conflation itself is [#514](https://github.com/WonderForgeLabs/gooey/issues/514).

**The layer's transparency stops at the layer.** It is the host that is `HitTestTransparent`, not what it holds — and since [#465](https://github.com/WonderForgeLabs/gooey/issues/465) an adornment that paints over something interactive also takes the press **and the hover** over it, because `DispatchMouse` derives both targets from one walk. Everything shipped declares `HitTestTransparent` and `TestEveryAdornmentIsHitTestTransparent` calls the method on each, so the exposure is the adorner somebody else writes: `Add` is exported and `Adornment` is an interface, so one that simply omits the method is opaque, at `OverlayRankAdornment` — the **top** rank — with no load error, no vet and no test to say so.

The duration is the half that is worse here than for a toast. A toast's swallowing is three seconds of an unclickable button; a `PersistentAdornment` is up for exactly as long as its anchor is invalid, so an opaque validation marker sits on the very field the user is trying to fix, and there is no timer to wait out. `components/adorn.go` carries the full argument and the remedy.
...
Two things that sentence does **not** promise. The designer does not follow it: `Grant.AttrsFor` decides the `Name` row from the spec, where the loader decides it from the call site, so a host `ParsedBy` def with a real `Build` gets no `Name` row in the property grid while the loader accepts the attribute and applies it — an attribute that works with no designer surface anywhere. The bit that would close it is not derivable: all three built-in pseudo-elements carry a `Build` too, and theirs exist only to refuse. And the attribute gate below reaches a child only where the parent's `Build` routes it through `markup.BuildChildren` — a host reader that walks `e.Children` itself, which is the shape `ParsedBy` describes, never calls it at all, and the framework exports no hook for it to. Both are pinned by tests rather than fixed: `TestTheDesignerOffersNoNameRowWhereTheLoaderHonoursOne` and `TestTheDesignerAndTheLoaderAgreeAboutAnUnknowableDef` — the second of which used to name a third gap and now guards its closure. That gap was the LAYOUT rows: `TakesLayout` derived them from the `Proto`, so a host def with a real `Build` and no `Proto` had `Margin`, `Width` and every attached name refused at load while `build()` honoured all of them. `ElementSpec.AxesKnown` separates "this type is not laid out" from "no type was given", so the loader no longer refuses a layout attribute the grid offers — for the def-backed specs `ElementSpec.Builds` scopes it to. The *other* direction is still open, and this sentence claimed both until review of [#486](https://github.com/WonderForgeLabs/gooey/pull/486) measured it: the grid offers `Margin`, `Width`, `Height` and the attached names for a def whose `Build` returns a component with no `Layout`, and the loader refuses every one of them (the refusal described two sentences on). Nothing in the catalog can anticipate that, since what `Build` returns is unknowable before it runs, so it is pinned rather than closed — `TestTheGridOffersLayoutRowsTheLoaderWillRefuse`. A registered component, an include control and an unknown element share `AxesKnown == false` and have no `Build` here at all, so they keep the narrower surface they had. What is left inside the arm is the def whose `Build` returns a component with **no** `Layout`, and that is answered by the loader rather than the catalog: `applyLayout` refuses the layout attributes by name instead of dropping them, since whether the built value implements `gooey.HasLayout` is unknowable until `Build` has run (`TestAnUnknowableDefThatBuildsNoLayoutIsRefusedByName`). `Name` and `Tooltip` are not refused — neither goes through the `Layout`.

**That last refusal is phrased over what the element BUILDS, and it runs only where the element's attribute surface is ENUMERABLE.** A name the element **declares** in its own `Attrs` and reads in its own `Build` is exempt — `Height` on `<Sparkline>` is that shape in this tree — because there the claim "accepted and never applied" would be false and removing it would break the element. The same reasoning is what scopes the refusal: it is gated on `ElementSpec.AttrsKnown`, so it does not fire for a `Context.Components` builder (whose spec does not resolve at all) or for a `Context.Elements` def with `Known: false` or `Opaque` set (whose `Attrs` is empty rather than exhaustive). On those surfaces the loader cannot know who reads the name, and a refusal advising "remove it" breaks the element whenever the builder was reading it — measured. The layout attribute is still silently dropped there; that is the remaining half, and it is [#550](https://github.com/WonderForgeLabs/gooey/issues/550) rather than this PR, because closing it needs a way for a builder to say which attributes it consumes. Pinned in both directions by `TestAnUnknowableDefThatBuildsNoLayoutIsRefusedByName` (`Bareish` refused, `Widthy` / `LogPane` / `Widthless` accepted). This paragraph claimed the opposite for one round — that the refusal reached the `Components` path as a deliberate breaking change — until review of [#486](https://github.com/WonderForgeLabs/gooey/pull/486) round 12 measured a builder consuming a name the refusal said it never would.

What a host declaration **does** get is the ordinary unknown-attribute gate, under the container the catalog says reads it — which need not be `ModeRestricted`, since a host container that hands its children to `markup.BuildChildren` enumerates nothing. `<Table><Row Label="a" Bogus="x"/></Table>` is refused with `<Row>`'s own vocabulary. Where the catalog *does* state a home and the document is somewhere else, the attribute fault is **held, not dropped**: the element's own `Build` reports the placement — the larger fault, and the one whose remedy fixes the file — and the held attribute fault is emitted only if that `Build` returns clean. So `<VStack><Row Margin="2" Label="a"/></VStack>` names the placement, and `<VStack><Built Label="a" Bogus="x"/></VStack>`, where the build succeeds, still names `Bogus`. This paragraph described an unconditional stand-down, which would have meant the second document loaded; `deferredFault` replaced that and `TestADeferralBuysNoSilenceWhereNoDiagnosisFollows` pins it. Both halves were regressions found in review of [#486](https://github.com/WonderForgeLabs/gooey/pull/486).
...
| `Visibility` | `Visible` (default), `Hidden`, `Collapsed`, or a `{{...}}` binding | Hidden occupies space and renders no content; Collapsed occupies nothing (and its subtree is skipped by focus traversal). **Neither is hit-tested** — a Hidden node is skipped by the hit walk since [#465](https://github.com/WonderForgeLabs/gooey/issues/465), so a press lands on whatever is beneath it. **The two skips are not the same size**, and this row said only Hidden's until review of [#458](https://github.com/WonderForgeLabs/gooey/pull/458): for Hidden only the NODE is skipped, so a Visible child of a Hidden parent is still on screen and still hittable, while Collapsed returns from `hitTest` ABOVE the bounds test, so a Visible child of a Collapsed parent is not hittable at all. That is the difference to weigh when choosing between the two values, which is what this page is for. One sharp edge worth knowing before you reach for Hidden over Collapsed: a hidden LEAF still pre-clears its own bounds, so it erases a visible sibling it overlaps ([#508](https://github.com/WonderForgeLabs/gooey/issues/508)). The bound form accepts a `*prop.Property[gooey.Visibility]` or a `*prop.Property[bool]` (true→Visible, false→Collapsed); a `Set` repaints exactly what the literal flip repaints. |
