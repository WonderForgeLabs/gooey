...
**Declare the `MenuBar` wherever it belongs.** The dropdown surface is a `gooey.Overlay`, so the Composer lifts it out of document order into a paint layer above the page and it covers the content it drops over from anywhere in the tree; in a `Grid`, `Grid.Row="0"` places the bar on the top row as usual.

*"Declare the `MenuBar` as the LAST child of its container"* is what this said, and [#430](https://github.com/WonderForgeLabs/gooey/issues/430) is the bug that disproved it: being last bought being above the bar's own siblings, so a `MenuBar` on the wysiwyg designer canvas — beside a `Gauge`, an `ItemsView` and a `Border` — had its dropdown painted over on the very next repaint. Position is free — for the bar, and since [#439](https://github.com/WonderForgeLabs/gooey/issues/439) for `ToastHost` and `AdornmentLayer` too, which are lifted and ranked. Within the layer the RANK orders, so a toast raised over an open menu is still readable.
...
**The freedom is about paint, not about clicks.** Hit-testing is a separate walk that prefers later siblings and knows nothing about ranks, so a host declared **first** paints its toasts above a button and leaves the press to the button. Toasts are informational, so this is usually nothing; if yours must be clickable where it overlaps something interactive, declare the host late. Tracked in [#465](https://github.com/WonderForgeLabs/gooey/issues/465), and held open by `gooey.TestARankOrdersPaintAndNotHitTesting` — citing it here is what puts this paragraph on the list that test's failure message prints, so closing the gap reaches this page rather than leaving it teaching a divergence that is gone. (A `Popup` is exempt: it holds pointer capture while open, which routes presses before the walk runs.) The host itself paints nothing and costs nothing while no toast is up; each toast is an ordinary leaf realized through the same structural re-sync a list uses, so showing one paints one component and dismissing one repaints exactly what it was covering.
