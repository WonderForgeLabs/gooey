...
The universal layout attributes are applied by the framework after the builder returns, so a custom component that embeds `gooey.Base` gets `Margin`, `Grid.Row`, and the rest for free. Letting a registered component declare its surface and have its attributes checked like a built-in's is [PR #290](https://github.com/WonderForgeLabs/gooey/pull/290).
