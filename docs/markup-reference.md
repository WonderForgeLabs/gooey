...
**That last refusal is phrased over what the element BUILDS, so it is wider than the `AxesKnown` arm above it and is a breaking change in its own right.** `applyLayout` runs for *every* element, and the catalog gate never reaches a `Context.Components` builder at all: its spec does not resolve, so `checkAttrs` returns early and `applyLayout` is the only gate there is. A host app with a custom component that does not embed `gooey.Base` — a documented extension point — therefore turns `<LogPane Margin="2"/>` from *loads and drops the margin* into a load error naming `Margin`. That is the silent drop being closed rather than a new restriction, and the remedy in the message is the real one: embed `gooey.Base` in what the builder returns, or stop writing the attribute. A name the element **declares** in its own `Attrs` and reads in its own `Build` is exempt — `Height` on `<Sparkline>` is that shape in this tree — because there the claim "accepted and never applied" would be false and removing it would break the element (`TestAnUnknowableDefThatBuildsNoLayoutIsRefusedByName`'s `Widthy` and `LogPane` arms). Raised in review of [#486](https://github.com/WonderForgeLabs/gooey/pull/486).
