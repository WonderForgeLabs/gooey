...
*"Declare the `MenuBar` as the LAST child of its container"* is what this said until [#430](https://github.com/WonderForgeLabs/gooey/issues/430), which is the bug that shows why it was never the rule: being last bought being above the bar's own siblings, so a `MenuBar` on the wysiwyg designer canvas — beside a `Gauge`, an `ItemsView` and a `Border` — had its dropdown painted over on the very next repaint. Position is free now. The one thing it still orders is **hit-testing**, which is not lifted; an open menu holds the pointer capture, so the bar does not depend on that either.
