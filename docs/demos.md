...
- Keys — **the document**: `d` toggle DESIGN/LIVE (the status bar names the mode and the key), `t` toggle theme, `ctrl+n`/`ctrl+p` next/previous element, `esc` select the parent, `x` delete, `alt+k`/`alt+j` move the selection up/down among its siblings, `alt+h`/`alt+l` promote out to the grandparent / demote into the sibling above, `ctrl+d` duplicate the selection and select the copy, `q`/`ctrl+c` quit
