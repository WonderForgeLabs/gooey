...
A terminal UI builder that edits gooey markup, laid out the way VS Code is: an activity rail, a side bar, the designer, a properties pane, a bottom panel and a status bar. The rail's fifth slot is the **docs tab**: it lists the repo's own `docs/` tree and shows the selected page as preformatted text, so the platform's documentation is readable inside the thing it documents. The page does not scroll yet — see the comment in `wysiwyg.gooey` and `docs/specs/2026-08-23-scrolling.md`. It is the component catalog's first real consumer, and it exists to test the SHAPE of that surface against running code rather than to be a finished editor.

Those panes are **dockable**, and that is a property-graph claim rather than a layout one. The arrangement used to be a `<Grid>` track list with one pane hardcoded per cell — and a Grid track list is a plain `[]GridLen` field read during `Arrange`, so nothing observes it and no gesture could move a pane without rewriting the markup and rebuilding the page. It is a model now: `<DockHost>` declares the panes, every dock gesture mutates the model, and the model is read *while painting*, which is what schedules the frame that re-lays it out. `<DockHost>`/`<DockPane>` are editor chrome registered by this app alone, not framework vocabulary — they are deliberately absent from `docs/markup-reference.md`.

Hidden, collapsed and pinned are three operations rather than three names for one. **Hidden** keeps the pane's slot space and its subtree — `gooey.Hidden` exactly, so a TextBox keeps its caret and a `Startable` keeps running. **Collapsed** shrinks to the header and gives the space back to its slot-mates; that is the one that reclaims room. It shrinks along the axis its slot STACKS on, so a pane in the side rails loses its rows down to the one header row, and a pane in the bottom strip — which stacks left to right — loses its columns down to the width its header draws in. The strip's own height falls to what the panes that are still open ask for — down to one header row when every pane in it is collapsed. A horizontal strip is ONE height and cannot be partly short, which is why the rule is the largest open pane rather than each pane's own ask; this page said the height came back "only when every pane in it is collapsed" until #441's review, and that left a header over 33 empty rows whenever the pane collapsed was the tallest one. **Pinned** changes nothing on its own and decides only what "hide everything unpinned" spares.
...
- Run: `cd apps/wysiwyg && go run .` — it is a nested module, so the root `./...` does not build it; `-attach 127.0.0.1:7777 -island Body` drives another app's control plane instead of previewing locally, `-serve`/`-mcp` set this editor's own endpoints (empty disables), `-graphics sixel|kitty|iterm2|cells` forces a pixel protocol, `-workspace <dir>` opens that directory as the workspace (empty starts with no folder), `-history N` sets how many undo steps are kept (0 disables `ctrl+z` entirely, which is the switch the undo tests use rather than a mock)
- Keys — **the document**: `d` toggle DESIGN/LIVE (the status bar names the mode and the key), `t` toggle theme, `ctrl+n`/`ctrl+p` next/previous element, `esc` select the parent, `alt+enter` select the first child (the inverse of `esc`, and the only way to reach a `<Menu>` or `<MenuItem>` — they build no component, so the pointer cannot hit one), `x` delete, `alt+k`/`alt+j` move the selection up/down among its siblings, `alt+h`/`alt+l` promote out to the grandparent / demote into the sibling above, `ctrl+d` duplicate the selection and select the copy, `q`/`ctrl+c` quit
- Keys — **clipboard and history**: `y` yank (copy) and `p` put (paste) — vi's spelling, because `ctrl+c` is already quit and a bare letter is safe on the root for the same reason `q` and `x` are — `ctrl+x` cut, `ctrl+z`/`ctrl+y` undo and redo
- Keys — **the dock**: `ctrl+alt+left`/`ctrl+alt+right` focus the previous/next pane, `ctrl+left`/`ctrl+right`/`ctrl+up`/`ctrl+down` move the focused pane to that slot (`ctrl+up` is the CENTRE, not "up"), `alt+up`/`alt+down` reorder it within its slot, `shift+up`/`shift+down` grow/shrink it, `ctrl+w` hide, `ctrl+r` collapse, `ctrl+alt+p` pin, `ctrl+f` hide everything unpinned, `ctrl+a` show all
- Keys — **the workspace and the preview source**: `ctrl+o` open a folder, `ctrl+s` save, `ctrl+\` swap the region under the designer, `ctrl+b` preview the built-in document, `alt+e` preview the editor's own page

  Two of those moved to avoid **silent** collisions, which is why they are not the obvious letters: pin is `ctrl+alt+p` — `alt+p` is the `_Project` menu's mnemonic and `ctrl+k` was the designer's MoveUp at the time — and hide-unpinned is `ctrl+f` because the status bar's endpoint chips took `ctrl+u`. Two `KeyBinding`s on one gesture at the same host is not a load error — dispatch offers them in order and the loser simply never fires — so a collision ships as "that key stopped working" with nothing red anywhere.

  The move cluster is on `alt+` for a different silent failure: `ctrl+h` and `ctrl+j` are the bytes `0x08` and `0x0a`, which the decoder reads as backspace and enter, so those two bindings never fired at all. `input.ParseGesture` rejects gestures no decoder can produce now (#427), which is what turned that silence into a load error.

  All of these live on the page ROOT, because a `KeyBinding` only fires while the focused chain passes through its host.

Undo is **derived, not recorded by hand**. Every mutator in the editor ends in one `rebuild()` — it has to, or the preview, the outline, the CODE tab and the build status all go stale — so the history is taken at that single choke point rather than by each mutator remembering to push a step. A mutation added later is undoable without its author knowing undo exists; the way it would fail is a mutator that quietly does not rebuild, which is why the round-trip test is a table over every mutator rather than a few spot checks.
...
Exercises the toolkit end to end: the Startable animation discipline shared by `ProgressBar`, `Spinner`, the toast auto-dismiss timers and the tooltip delay (post, never touch the graph from the goroutine; stop closes and joins), rocker arrow semantics that consume a key only when it moves something, `ButtonBar` uniform sizing and its `gooey.FocusHost` focus scope, pixel button chrome placed per paint node, `Validate` behaviors publishing error properties a computed can gate a command on — and the overlay story: the `MenuBar`'s dropdown, the `ToastHost` and the `AdornmentLayer` are lifted out of document order into a paint layer of their own and ranked within it, so they paint above every tab from wherever their hosts sit and a toast raised over an open menu is still readable — the BAR is not lifted and never was, which is why `Grid.Row` still keeps it on the top row; the open menu holds the pointer capture so a click elsewhere dismisses without activating what is underneath, and dismissing any overlay — menu, toast, tooltip, or popup — repaints exactly the components it was covering (the Composer's restore pass).
...
**The GIF predates the menu icons and has not been re-recorded** — the Job menu now draws `▶ ■ ↺ ✕` and Notify draws `●` from `MenuItem`'s `IconRune`, none of which is in the capture. A re-record is the honest illustration here rather than a nicety: a pty reports no graphics protocol, which is exactly the tier `IconRune` exists to serve, so this is the one demo where the recording shows the feature rather than routing around it. Pending the docs-and-demos workflow, as the `typeahead` entry is.
...
and the MenuBar overlay recipe reused in an app: the popup surface
lifted into the overlay layer (so where the picker sits does not decide
what it paints over), modal focus with key swallowing, pointer capture
while open, focus restored on dismiss. All git work (enumeration, `worktree add`
