...
Hidden, collapsed and pinned are three operations rather than three names for one. **Hidden** keeps the pane's slot space and its subtree — `gooey.Hidden` exactly, so a TextBox keeps its caret and a `Startable` keeps running. **Collapsed** shrinks to the header row and gives the space back to its slot-mates; that is the one that reclaims room. **Pinned** changes nothing on its own and decides only what "hide everything unpinned" spares.
