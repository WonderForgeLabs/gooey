...
- Keys: `d` toggle DESIGN/LIVE (the status bar names the mode and the key), `ctrl+n`/`ctrl+p` next/previous element, `esc` select the parent, `x` delete, `ctrl+k`/`ctrl+j` move the selection up/down among its siblings, `ctrl+h`/`ctrl+l` promote out to the grandparent / demote into the sibling above, `ctrl+d` duplicate the selection and select the copy, `q`/`ctrl+c` quit; the bindings live on the page ROOT because a `KeyBinding` only fires while the focused chain passes through its host
