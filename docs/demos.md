...
Exercises the toolkit end to end: the Startable animation discipline shared by `ProgressBar`, `Spinner`, the toast auto-dismiss timers and the tooltip delay (post, never touch the graph from the goroutine; stop closes and joins), rocker arrow semantics that consume a key only when it moves something, `ButtonBar` uniform sizing and its `gooey.FocusHost` focus scope, pixel button chrome placed per paint node, `Validate` behaviors publishing error properties a computed can gate a command on — and the overlay story: the `MenuBar`'s dropdown, the `ToastHost` and the `AdornmentLayer` are lifted out of document order into a paint layer of their own, so they paint above the page from wherever they are declared — they sit at the end of the root `Grid` because that is where they read best, and `Grid.Row` is what keeps the bar on the top row (this clause said "document order is z-order, so … are declared as the root Grid's *last* children" until review of #455; that is the rule [#430](https://github.com/WonderForgeLabs/gooey/issues/430) disproved), the open menu holds the pointer capture so a click elsewhere dismisses without activating what is underneath, and dismissing any overlay — menu, toast, tooltip, or popup — repaints exactly the components it was covering (the Composer's restore pass).
