...
The toolkit on one page, spelled entirely in markup: the six wave-1 components plus wave 2's overlays — a `MenuBar` over the content and a `ToastHost` popping notifications above everything.

The walkthrough: the job opens in its fetch stage, which has no measurable progress — so the `ProgressBar` marches a band instead of claiming a number, and the `Spinner` turns beside it. `shift+tab` reaches the `MenuBar` and `enter` drops the Job menu **over** the content rows, gesture hints right-aligned per item; `↓` walks the items (the separator is skipped), `→` slides the dropdown to the Notify menu — the cells the first dropdown vacated repaint from what was beneath — and `enter` on "Toast the status" closes the menu and pops a toast in the top-right corner, which takes itself down 2.5s later and leaves no scar. `esc` closes an open menu and restores focus to whatever had it. Meanwhile the build finishes on its own; the `ButtonBar`'s `toast` member fires the same notification, and `abort` — a `NewCommand(...).When(Running)` — goes dim the moment the job stops. The `StatusBar` along the bottom is bound the whole time: status on the left, a clock in the middle, key hints on the right, each its own paint node.
...
- Keys: `tab` move focus, `enter`/`↓` open the focused menu, `esc` close it, `←`/`→` traverse bars and menus, `space` toggle, `ctrl+t` toast, `q` quit

Exercises the toolkit end to end: the Startable animation discipline shared by `ProgressBar`, `Spinner`, and the toast auto-dismiss timers (post, never touch the graph from the goroutine; stop closes and joins), rocker arrow semantics that consume a key only when it moves something, `ButtonBar` uniform sizing and its `gooey.FocusHost` focus scope, pixel button chrome placed per paint node — and the overlay story: document order is z-order, so the `MenuBar` and `ToastHost` are declared as the Grid's *last* children while `Grid.Row` keeps the bar on the top row, the open menu holds the pointer capture so a click elsewhere dismisses without activating what is underneath, and dismissing any overlay repaints exactly the components it was covering (the Composer's restore pass).
