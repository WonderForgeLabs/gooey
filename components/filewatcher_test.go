...
//
// WHAT EACH ROW COVERS: the three watcher callers all pass w.Interval
// from a watcher declaring Interval: time.Millisecond, so they take the
// 50ms floor; the zero row's caller clamps to DefaultWatchInterval,
// which is above the floor, so the scaling branch runs on every run of
// this suite; and the 200ms row is the only coverage of an above-floor
// interval a CALLER chose. Written as a table rather than as a
// converted caller because what is under test is arithmetic, and a
// converted caller would pay two seconds of real waiting for it.
...
func TestCountingPostEnqueuesBeforeItCounts(t *testing.T) {
	...
	// AND THE OTHER ASYMMETRY, which the doc states as a contract and
	// nothing pinned: a nil DELEGATE panics where a nil fn is dropped.
	// Stated and unasserted is the shape this branch has spent its
	// rounds removing, and the panic is the whole reason the zero value
	// is called unusable rather than empty. Raised in review of #511.
	func() {
		defer func() {
			if recover() == nil {
				t.Error("a countingPost with no post swallowed a post: the zero " +
					"value must panic, or a fixture that forgets the delegate " +
					"counts posts nobody made and passes")
			}
		}()
		(&countingPost{}).Post(func() {})
	}()
}
...
func TestFileWatcherDoesNotFireOverAnUnchangedFile(t *testing.T) {
	...
	// at all, and a handful of cycles says only that a spurious fire
	// does not happen immediately — which a mis-seeded baseline passes
	// as readily as a correct one. n is the coverage, not a margin.
	//
	...
	// wall-clock window this replaced could not buy the forty polls its
	// message named even with nothing else running.
...
func TestAFileChangeSchedulesAFrameAndAnIdlePollDoesNot(t *testing.T) {
	...
	// THE FIXTURE OWNS THE WATCHER'S LIFETIME, not the Composer, and
	// that is the whole reason this assertion has a floor. comp.Start(d)
	// hands FileWatcher.Start the Dispatcher's own Post
	// (composer.go), leaving nothing to count, so this used to wait a
	// wall-clock window — and a window that buys no polls satisfies a
	// NEGATIVE assertion without the watcher having run at all, which is
	// the flake class this branch exists to remove. Composer.disp is
	// read only to start and stop Startables, so starting the watcher
	// here costs the fixture nothing it asserts on; that the Composer
	// starts an attached watcher is
	// TestComposerStartsAndStopsAnAttachedFileWatcher's claim, not this
	// one's. Raised in review of #511.
	c := &countingPost{post: d.Post}
	stop := fw.Start(c.Post)
	defer stop()
	...
	posts := drainUntilPosts(t, d, c, 40, fw.Interval)
	if scheduled != 0 {
		t.Fatalf("an idle watcher asked for %d frames over %d poll posts; it must cost nothing",
			scheduled, posts)
	}
...
func TestAFileChangeReachesTheCellsAndCostsAWireUpdate(t *testing.T) {
	...
	// The fixture starts the watcher for the reason the schedule test
	// gives, and the zero-bytes claim below is the same shape as the
	// zero-schedules one: negative, so it needs a floor made of the
	// watcher's own cycles rather than a wall-clock window.
	c := &countingPost{post: d.Post}
	stop := fw.Start(c.Post)
	defer stop()

	drainUntilPosts(t, d, c, 40, fw.Interval)
	comp.Frame()
	sink.Reset()
	if err := comp.Flush(&sink); err != nil {
		t.Fatal(err)
