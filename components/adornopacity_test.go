...
// rank, above popups and above toasts. It takes the press over the field
// it is pinned beside, for exactly as long as its anchor is invalid, and
// there is no load error, no vet and no test to say so.
//
	...
	for _, a := range adornments {
		if !methods[a]["HitTestTransparent"] {
			t.Errorf("%s declares Anchor and Place — it is an Adornment — and "+
				...
				"over whatever it is pinned beside for as long as its anchor is "+
				"invalid. See AdornmentLayer.HitTestTransparent in adorn.go", a)
		}
	}
}
