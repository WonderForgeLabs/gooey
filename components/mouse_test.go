...
// pane is an app-style component: focusable, hoverable, and it records the
...
func (p *pane) Render(f *gooey.Frame) { p.IsFocused(); p.IsHovered() }
...
func TestHitTestDeepestWins(t *testing.T) {
	...
	nested := &VStack{Children: []gooey.Component{inner}}
	sibling := &Text{Content: Str("y")}
	root := &HStack{Children: []gooey.Component{nested, sibling}}
	m := gooey.NewFocusManager(root)
	root.Measure(gooey.Size{W: 20, H: 4})
	root.Arrange(gooey.Rect{X: 0, Y: 0, W: 20, H: 4})

	if got := m.HitTest(inner.Bounds().X, inner.Bounds().Y); got != gooey.Component(inner) {
		t.Fatalf("hit inside nested = %T, want the innermost Text", got)
	}
	if got := m.HitTest(sibling.Bounds().X, sibling.Bounds().Y); got != gooey.Component(sibling) {
...
func TestHitTestOverlapPrefersLastPainted(t *testing.T) {
	...
	root := &Grid{Children: []gooey.Component{under, over}} // one cell, both fill it
	m := gooey.NewFocusManager(root)
	root.Measure(gooey.Size{W: 10, H: 3})
	root.Arrange(gooey.Rect{X: 0, Y: 0, W: 10, H: 3})

	if got := m.HitTest(5, 1); got != gooey.Component(over) {
...
func TestHitTestSkipsCollapsed(t *testing.T) {
	...
	root := &Grid{Children: []gooey.Component{shown, hidden}} // hidden would be on top
	m := gooey.NewFocusManager(root)
	root.Measure(gooey.Size{W: 10, H: 3})
	root.Arrange(gooey.Rect{X: 0, Y: 0, W: 10, H: 3})

	if got := m.HitTest(5, 1); got != gooey.Component(shown) {
		t.Fatalf("hit = %p, want the visible component %p", got, shown)
	}
}

func TestPressMovesFocus(t *testing.T) {
	top, bottom := &pane{}, &pane{}
	c := gooey.NewComposer(&VStack{Children: []gooey.Component{top, bottom}}, 20, 4)
	c.Frame()
	if c.Focus().Focused() != gooey.Component(top) {
		t.Fatal("focus should start on the first pane")
	}
	c.HandleMouse(press(3, bottom.Bounds().Y))
	if c.Focus().Focused() != gooey.Component(bottom) {
		t.Fatal("press did not move focus to the component under the pointer")
	}
	// Focus damage is the same two-component repaint as tab.
	if _, painted := c.Frame(); painted != 2 {
		t.Fatalf("focus-follows-click painted %d components, want 2", painted)
	}
}
...
func TestWheelGoesToPointerNotFocus(t *testing.T) {
	...
	if len(top.got) != 0 {
		t.Fatal("wheel reached the focused component instead of the hovered one")
	}
	if c.Focus().Focused() != gooey.Component(top) {
...
func TestHoverTransitionFlipsExactlyTwo(t *testing.T) {
	...
	if _, painted := c.Frame(); painted != 1 {
		t.Fatalf("entering the first component painted %d, want 1", painted)
	}
	...
	if _, painted := c.Frame(); painted != 2 {
		t.Fatalf("hover move painted %d components, want 2 (leave + enter)", painted)
	}
	// Moving within the same component is not a transition: no damage.
	c.HandleMouse(move(5, bottom.Bounds().Y))
	if _, painted := c.Frame(); painted != 0 {
		t.Fatalf("moving inside one component painted %d, want 0", painted)
	}
}

// Raw motion is not delivered to ordinary components — only enter/leave is.
func TestMoveNotDeliveredWithoutMoveHandler(t *testing.T) {
	p := &pane{}
	c := gooey.NewComposer(&VStack{Children: []gooey.Component{p}}, 20, 4)
	c.Frame()
	c.HandleMouse(move(3, p.Bounds().Y))
	if len(p.got) != 0 {
		t.Fatalf("component received raw motion %v; it did not ask for it", p.got)
	}
}
...
func TestClickSynthesis(t *testing.T) {
	...
	if len(top.got) != 3 || top.got[2] != input.MouseClick {
		t.Fatalf("press+release on one component gave %v, want %v", top.got, want)
	}

	// Press on one component, release on another: the release still goes to
	// the pressed component (implicit capture) but no Click is synthesized.
	...
	for _, k := range append(top.got, bottom.got...) {
		if k == input.MouseClick {
			t.Fatal("Click synthesized across two different components")
		}
	}
}
...
func TestButtonClickAndVisualStates(t *testing.T) {
	...
	if _, painted := c.Frame(); painted != 1 {
		t.Fatalf("pressed state painted %d components, want 1", painted)
	}
...
func TestHoverUsesNearestHoverableAncestor(t *testing.T) {
	...
	c := gooey.NewComposer(&VStack{Children: []gooey.Component{hoverable, p}}, 20, 4)
	c.Frame()

	c.HandleMouse(move(1, child.Bounds().Y))
	if c.Focus().HitTest(1, child.Bounds().Y) != gooey.Component(child) {
...
func (h *hoverBox) ChildComponents() []gooey.Component  { return []gooey.Component{h.Child} }
func (h *hoverBox) Measure(avail gooey.Size) gooey.Size { return gooey.MeasureChild(h.Child, avail) }
func (h *hoverBox) Arrange(r gooey.Rect)                { h.Base.Arrange(r); gooey.ArrangeChild(h.Child, r) }
func (h *hoverBox) Render(f *gooey.Frame)               { h.IsHovered() }
