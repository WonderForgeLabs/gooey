...
//
// FocusManager.setHover (mouse.go) drives hover on the hit, so when
...
// bubbling to ancestors (DispatchMouse's MouseMove arm, in mouse.go) —
// which is what a drag, a marquee
