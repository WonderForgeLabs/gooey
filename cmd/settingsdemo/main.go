// settingsdemo: the three settings the browser wants — last source,
...
func main() {
	...
	if path == "" {
		...
		path = filepath.Join(dir, "gooey", "settingsdemo.json")
	}
	...
	dir := "cmd/settingsdemo"
	if _, err := os.Stat(filepath.Join(dir, "settingsdemo.gooey")); err != nil {
		exe, _ := os.Executable()
		dir = filepath.Dir(exe)
	}

	app = gooey.NewApp(markup.Page(os.DirFS(dir), "settingsdemo.gooey", ctx))
