// typeaheaddemo puts <TypeAhead> — Windows Explorer's type-ahead find —
...
//
//	typeaheaddemo                 detect the graphics protocol, run
//	typeaheaddemo --mode=sixel    force one (kitty|sixel|iterm2|halfblock)
//	typeaheaddemo --dump          one frame to stdout, no tty control
//	typeaheaddemo --hold=3s       exit after a while instead of on a key
//
...
const pageFile = "typeaheaddemo.gooey"

// pageDir is the demo's own directory, so `go run ./cmd/typeaheaddemo`
