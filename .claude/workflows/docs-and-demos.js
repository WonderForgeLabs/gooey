...
- Convert with: agg --theme dracula --font-size 14 out.cast ${REPO}/<demo>.gif
...
- Final GIF must land at ${REPO}/<demo>.gif (overwriting is correct).`
...
const RECORDERS = [
  ...
  {
    demo: 'propdemo',
    brief: `Re-record cmd/propdemo (dependency-property graph demo). Old script: ${S}/record-props.sh (adapt: binary is now ${S}/propdemo and the stats line now reads "widgets painted last frame=N"). Choreography: ~2s idle (1 Hz tick renders), hammer 'b' 5x while unwatched (NO frames until next tick - the point), press 'a' (instant), press 'm' (toggle watched source), press 'b' twice (now instant), 'q'. 80x24. Expected strings in cast: "widgets painted last frame", "detail evals". The money proof: events counter jumps +5 in one hop while frames barely move, and painted counter shows ~2 of 8.`,
  },
  {
    demo: 'logview',
    brief: `Re-record cmd/logview (log viewer with pause via conditional dependency). Old script: ${S}/record-logview.sh (binary ${S}/logview; stats line now includes "widgets painted last frame"). Choreography: ~3s following (stream renders), space (pause - screen freezes while lines keep arriving), 'f' cycle filter twice WHILE PAUSED (UI still alive against frozen snapshot), 'f' back to all, space (resume - one-frame catch-up), 'q'. 100x28. Expected: "PAUSED", "filter: ERROR", "lines arrived". Money proof: lines-arrived jumps ~60 while frames rose ~5 during the pause.`,
  },
  {
    demo: 'markuplog',
