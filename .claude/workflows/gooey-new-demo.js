...
- The tools: \`screen_text\` (your screenshot — quote it in results and in AskUserQuestion previews), \`screen_size\` (how many cells you have and where their origin is — the space send_mouse coordinates are in — without fetching a whole tree_snapshot; it also reports the terminal's pixel-per-cell metrics, 0 when nobody probed, and narrows to the island for a scoped session), \`tree_snapshot\`, \`swap_markup\` / \`patch_markup\` / \`validate_markup\` (change the UI without restarting), \`send_keys\`, \`send_mouse\`, \`focus\`, \`set_value\` / \`list_values\`, \`invoke_command\`, \`list_styles\`, \`register_properties\` / \`unregister_properties\`.
...
5. No stray binaries or junk: git status --porcelain -uall — every untracked path UNDER ${build.location} (and any other path this workflow reported touching) must be an intended demo file; flag executables or logs there. \`-uall\` is load-bearing: it descends into untracked directories, where plain porcelain stops at the directory name and hides the files inside. Scope the judgement to this workflow's own paths: the shared checkout also carries other sessions' untracked files, and those are neither yours to explain nor yours to fail on — list them as a note.
6. Worktree hygiene (a NOTE, never a green gate — this workflow creates no worktrees, and what is under that directory belongs to the rest of the fleet): for each ENTRY in ${REPO}/.claude/worktrees/, check whether it holds a \`.git\` file or directory (\`ls -a ${REPO}/.claude/worktrees/*/.git\`). One that does not is not a worktree at all — git cannot see it via \`git worktree list\` OR \`git status\`, which is how real work has been stranded in this repo before. Do NOT compare the directory listing to \`git worktree list\` as sets: that list also carries the main checkout and worktrees registered outside this directory, so the two can never match. Report any \`.git\`-less directory under checks as a note; do not delete anything yourself, and do not let it change green.
green=true only if checks 1-5 pass (6 is a note).${INVARIANTS}${GIT_RULES}`,
