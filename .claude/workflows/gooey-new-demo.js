...
- The tools: \`screen_text\` (your screenshot — quote it in results and in AskUserQuestion previews), \`screen_size\` (the terminal's cell size, and a scoped session's island size), \`tree_snapshot\`, \`swap_markup\` / \`patch_markup\` / \`validate_markup\` (change the UI without restarting), \`send_keys\`, \`send_mouse\`, \`focus\`, \`set_value\` / \`list_values\`, \`invoke_command\`, \`list_styles\`, \`register_properties\` / \`unregister_properties\`.
