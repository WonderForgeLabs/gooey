...
- The tools: \`screen_text\` (your screenshot — quote it in results and AskUserQuestion previews), \`screen_size\` (how many cells you have and where their origin is — the space send_mouse coordinates are in — without fetching a whole tree_snapshot; it also reports the terminal's pixel-per-cell metrics, 0 when nobody probed, and narrows to the island for a scoped session), \`tree_snapshot\`, \`swap_markup\` / \`patch_markup\` / \`validate_markup\`, \`send_keys\`, \`send_mouse\`, \`focus\`, \`set_value\` / \`list_values\`, \`invoke_command\`, \`list_styles\`, \`register_properties\` / \`unregister_properties\`.
