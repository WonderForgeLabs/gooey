...
| Validation | done | Validators are computeds — `validate.Field(src, Required, Len, Pattern, Range…)` yields an error property (empty = valid, first failure wins), `validate.All` a value-stabilized is-valid for submit gating via `.When()`. Markup: a `<Validate Required="true" MinLen="3"/>` behavior on the input (bare child or `<X.Behaviors>` slot) builds the same computed, publishes it (`Into`) for inline error `<Text>`s, and wires the TextBox's invalid visual; `ctx.Rules` registers app rules for markup use; `<ValidationMarker/>` floats the message via the AdornmentLayer for dense layouts ([spec](docs/specs/2026-08-10-validation-core.md)) |
