...
| Menus, toasts, popups | done | `MenuBar` (modal dropdowns, page-wide `alt+letter` mnemonics, focus restored on dismiss) and `ToastHost` (auto-dismissed notifications), both declared as the *last* children of the root. `ToastHost` is itself a `gooey.Overlay`; `MenuBar` is not — it is an ordinary component whose dropdown *surface* is the overlay. Either way what they contribute paints in the overlay layer above the page, and declaration order is what ranks them against each other ([spec](docs/specs/2026-09-01-overlay-hosts.md)). The anchored-surface lifecycle — focus save/restore, pointer capture, dismissal grammar — is extracted as the Go-side `Popup` primitive ([spec](docs/specs/2026-08-10-popup.md)), and `MenuBar` is its one full adopter: `Tooltip` takes the placement helper (`PlacePopup`) and hand-rolls the rest, and `ToastHost` does not adopt it at all, both by design. There is no markup `<Popup>` element yet; menus v2 — context menus and submenus — is [#104](https://github.com/WonderForgeLabs/gooey/issues/104) |
