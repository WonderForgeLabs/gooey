...
| Adornments + Tooltip | done | WPF's adorner plane: an `AdornmentLayer` (anywhere spanning the page — it ranks itself to the top of the overlay layer for PAINT, but re-anchors during its own `Arrange`, so declare it after the content it adorns if you write a CUSTOM adornment that neither persists nor follows the pointer; a `Tooltip` attaches on hover and a `ValidationMarker` persists, so neither can meet it: [#514](https://github.com/WonderForgeLabs/gooey/issues/514)) hosts components positioned against a *target's* arranged bounds — re-anchored every frame, dropped when the target vanishes. `Tooltip` is the first customer: `<Tooltip Text="…"/>` as a KeyBinding-style attachment or `Tooltip="…"` on any element, delay timer, flip-to-fit, gesture hints, pure markup ([spec](docs/specs/2026-08-10-adornments.md)). Free-position adornments ([#177](https://github.com/WonderForgeLabs/gooey/issues/177)) close the gap the layer shipped with: an adornment implementing `gooey.PointerFollower` is exempt from the anchor sweep and has `Place` called with the pointer's cell, so drag ghosts, drop indicators, marquees and crosshairs are expressible. The wake is a Composer observer (`armPointer`), not a paint node, and it is armed only while something is actually following — pointer motion with no gesture up schedules **zero** frames ([spec](docs/specs/2026-08-23-free-adornments.md)). `DragGhost` is the first customer |
...
| Menus, toasts, popups | done | `MenuBar` (modal dropdowns, page-wide `alt+letter` mnemonics, focus restored on dismiss) and `ToastHost` (auto-dismissed notifications). `ToastHost` is itself a `gooey.Overlay`; a `MenuBar`'s *dropdown* is one and the bar is not, which is why `Grid.Row` still places the bar and nothing places the dropdown. What carries the marker is lifted out of document order into a paint layer of its own, so it goes above the page from *wherever* its host is declared, and ranked within that layer so a toast is never hidden by an open dropdown ([spec](docs/specs/2026-08-30-overlay-layer.md), [ranks](docs/specs/2026-09-05-overlay-ranks.md)). The anchored-surface lifecycle — focus save/restore, pointer capture, dismissal grammar — is extracted as the Go-side `Popup` primitive ([spec](docs/specs/2026-08-10-popup.md)), and `MenuBar` is its one full adopter: `Tooltip` takes the placement helper (`PlacePopup`) and hand-rolls the rest, and `ToastHost` does not adopt it at all, both by design. There is no markup `<Popup>` element yet; menus v2 — context menus and submenus — is [#104](https://github.com/WonderForgeLabs/gooey/issues/104) |
