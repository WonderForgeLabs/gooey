...
| Menus, toasts, popups | done | `MenuBar` (modal dropdowns, page-wide `alt+letter` mnemonics, focus restored on dismiss) and `ToastHost` (auto-dismissed notifications), both declared as the last children of the root — though only `ToastHost` needs to be. A `MenuBar`'s dropdown is a `Popup` surface, and a `Popup` surface implements `gooey.Overlay`, so it is lifted out of document order into a paint layer above the page and lands on top wherever the bar sits ([#430](https://github.com/WonderForgeLabs/gooey/issues/430) is the bug that established this, on a designer canvas whose menu was painted over by a later sibling). `ToastHost` has no such lift yet, so for it document order is still the whole rule. The anchored-surface lifecycle — focus save/restore, pointer capture, dismissal grammar — is extracted as the Go-side `Popup` primitive ([spec](docs/specs/2026-08-10-popup.md)), and `MenuBar` is its one full adopter: `Tooltip` takes the placement helper (`PlacePopup`) and hand-rolls the rest, and `ToastHost` does not adopt it at all, both by design. There is no markup `<Popup>` element yet; menus v2 — context menus and submenus — is [#104](https://github.com/WonderForgeLabs/gooey/issues/104) |
