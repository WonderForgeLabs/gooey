// paintdemo draws with paint/ from markup: three pages of plates whose
...
func main() {
	...
	// boundary, so this is `cd paint && go run ./cmd/paintdemo`.
	dir := flag.String("dir", "cmd/paintdemo", "directory holding the .gooey pages")
	...
	app := gooey.NewApp(
		markup.Page(fsys, "paintdemo.gooey", ctx, "strokes.gooey", "brushes.gooey", "ring.gooey"),
		opts...)
	vm.app = app
...
// encoderFor mirrors cmd/demo's flag, with one name changed: "cells"
