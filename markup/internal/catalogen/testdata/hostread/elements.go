...
func buildHost(e Element, ctx *Context) (gooey.Component, error) {
	...
	for _, c := range e.Children {
		...
		_ = c.Attrs["Text"]
		// A UNIVERSAL read off a pseudo child, declared by nobody. The
		// under-declared half used to skip the eight universal names,
		// so this was waved through — while vocabulary() will never let
		// markup set Margin on a pseudo-element, making it a dead read.
